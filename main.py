...
def _build_format_list(formats: list) -> list:
    """
    整理成前端友好的格式列表。
    只选已含音视频的单一格式（无需 ffmpeg 合并）。
    """
    seen_heights = set()
    result = []

    # 优先：同时含音视频的单文件格式（mp4/webm）
    combined = [
        f for f in formats
    ...
    for f in combined:
        ...
        result.append({
            "format_id": f["format_id"],   # 直接用具体 format_id，不合并
            "label": label,
            "ext": f.get("ext", "mp4"),
...
async def download_video(req: DownloadRequest, background_tasks: BackgroundTasks):
    ...
    ext = filepath.suffix or (".mp3" if is_audio else ".mp4")

    # 用 RFC 5987 编码支持中文文件名，同时保证 latin-1 兼容的 fallback
    import urllib.parse
    encoded_name = urllib.parse.quote(filepath.name, safe="")
    # fallback 文件名只含 ASCII
    ascii_name = filepath.name.encode("ascii", "ignore").decode("ascii").strip()
    if not ascii_name:
        ascii_name = f"video_{file_id[:8]}{ext}"
    ...
    # 不传 filename 参数，避免 FileResponse 再生成一个 Content-Disposition 头
    return FileResponse(
        path=str(filepath),
...
async def download_video_get(url: str, format_id: str, background_tasks: BackgroundTasks):
    """
    GET 方式下载视频，前端直接 window.open() 触发，无 CORS 问题
    """
